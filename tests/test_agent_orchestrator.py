...
def make_request(**kwargs):
    values = {
        "message": "Login returns 401, and this order was charged twice.",
        "user_id": "u1",
        "conv_id": "c1",
        ...
        "entities": {"error_code": ["401"], "amount": ["99 yuan"]},
    }
...
def test_escalation_agent_is_a_real_non_llm_handoff_node():
    ...
    assert "manual escalation" in result.content
    assert client.calls == []
...
def test_composer_fallback_preserves_primary_and_supporting_results():
    ...
    req = make_request()
    responses = [
        AgentResponse(AgentType.TECHNICAL, "First check whether the token has expired.", True),
        AgentResponse(AgentType.BILLING, "Please provide the time and amount of both charges.", True),
    ]

    content = asyncio.run(composer.compose(req, responses))

    assert content.startswith("First check whether the token has expired.")
    assert "Additional instructions" in content
    assert "both charges" in content
    ...
    class RagManager:
        async def search_with_rewrite(self, tool_name, query, top_k=5):
            return type(
                "Result",
                (),
                {"success": True, "data": [{"title": "Refund Policy", "content": "7  Refundable within days"}], "reranked": True},
            )()
...
def test_tool_input_validation_rejects_unknown_fields():
    ...
    try:
        ...
        assert " Tool parameter not allowed" in str(exc)
    else:
...
def test_tool_use_round_trip_executes_only_whitelisted_tool():
    ...
    class TextBlock:
        type = "text"
        text = " Troubleshooting suggestions have been given based on the 401 error code."
